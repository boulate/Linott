...
<table border=0 width=100% id='tableBleu'>
	<tr>
		<td width=25%>
			...
			?>
           			<option value="<?php echo $nom_site ?>"<?php echo $selected ?>><?php echo $nom_site ?></option>	
			<?php
			}
		...
		<td width=25%>
			...
			?>
           			<option value="<?php echo $nom_marque ?>"<?php echo $selected ?>><?php echo $nom_marque ?></option>	
			<?php
			}
	...
	<tr>	
		<td>	<!-- Gestion du materiel / logiciel -->
			<?php>			
			...
			$id_materiel=$donnees['ID_materiel'];
				// si le "en cours" = "celui en base", alors selected="selected"
				if ($nom_materiel == $materiel_logiciel) {
					$selected = "selected=\"selected\"";
								}
				else {
					$selected = "";
			...
			?>
           			<option value="<?php echo $nom_materiel ?>"<?php echo $selected ?>><?php echo $nom_materiel ?></option>	
			<?php
			}
		...
		<td>
			...
			<form method="post" action="ajouter_materiel.php" enctype="multipart/form-data" target="_blank">
				<input type="text" size=16 name="new_materiel">
			        <input type="submit" value="Ajouter un materiel">   
			</form>
		</td>
		...
		<td>
			...
			?>
           			<option value="<?php echo $nom_type ?>"<?php echo $selected ?>><?php echo $nom_type ?></option>	
			<?php
			}
		...
		<td>
			...
			<form method="post" action="ajouter_type_materiel.php" enctype="multipart/form-data" target="_blank">
				<input type="text" size=16 name="new_type_materiel">
			        <input type="submit" value="Ajouter un type">   
			</form>
		</td>
	</tr>
	...
	<tr>
		...
		<td> <!-- Gestion du numero de serie -->
			...
			?>
           			<option value="<?php echo $num_serie ?>"<?php echo $selected ?>><?php echo $num_serie ?></option>	
			<?php
			}
		...
		<td>
			...
			<form method="post" action="ajouter_num_serie.php" enctype="multipart/form-data" target="_blank">
				<input type="text" size=16 name="new_num_serie">
			        <input type="submit" value="Ajouter un n° de serie">   
			</form>
		</td>
