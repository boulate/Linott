...
//require("checkAdmin.php");
	...
	{
		// Declaration des variables
		$activerSections	= $donnees['valeur'];

		if ( $activerSections == 1 )
		{
			$activerSections = "checked";
		}
		else
		{
			$activerSections = "";
	...
	///////////////////////////////////////////   ///////////////////////////////////////////
	$sql		= "SELECT valeur FROM Configuration where nom like 'filtrer_choix_axes'";
	//echo "$sql";
	$reponse 	= $bdd->query($sql) or die('<br>Erreur SQL au moment de la selection sur from fiche!<br>Votre selection semble ne correspondre a aucun resultat.' .$sql. '<br>'. mysql_error());
	while ($donnees = $reponse->fetch())
	{
		// Declaration des variables
		$filtrerChoixAxes		= $donnees['valeur'];

		if ( $filtrerChoixAxes == 1 )
		{
			$filtrerChoixAxes = "checked";
		}
		else
		{
			$filtrerChoixAxes = "";
